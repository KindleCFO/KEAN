...
from read_csv import read_lmp_csv
from scraping import get_file_pjm
...
changedate_csv = datetime.datetime.strptime('2017-01-14', '%Y-%m-%d')   #date files change to .csv
...
for day in range(0, endcount):          #13 years of data
    ...
    filename = fileyear + filemonth[-2:] + fileday[-2:] + '-da' + extension

    #get file from pjm-da
    get_file_pjm(filename)

    #reade file, laod into kean
    zipped_csv_filename = filename[:-3] + 'csv'

    if filedate < changedate_csv:
        read_lmp_zip(zipped_csv_filename)
    else:
        read_lmp_csv()
